...
Original file is located at
    https://colab.research.google.com/drive/1wUE-bh5jt2FUb3qNS-8vwULDMUA6IKGl

# **保存をしない**、Test速度のための調査用
...
def load_trained_data(path,expIndex,mesh_code,inSize):
...
def test_GR(path,res_params,expIndex,Distance,Rlist_dict):
  s_time = time.time()
  print(str(s_time) + "Programe Start...")
  #trainを全てのセルで終えてる前提
  ...
  now_time = time.time()
  print(str((now_time - s_time)//1) + "s " + "LargeGeoReservoir Start...")
  ...
  for t,r in enumerate(Rlist):
    ...
    tmp = load_trained_data(path,expIndex,r,inSize)
    ...
    print(str((time.time() - sst) *10 //1 /10) + "s "+str(rate) +"% done")

  gc.collect()
  now_time = time.time()
  print(str((now_time - s_time)//1) + "s " + "Loaded and Initialized...")
  ...
  gc.collect()
  now_time = time.time()
  print(str((now_time - s_time)//1) + "s " + "Coputed !!")
  print("Saving...")
...
res_params = (1, 1000, 0.75, 9, 9, 24*60, 3*24*60, 2*24*60-60+1, 1e-8)
expIndex = -9.5
dis = 30
